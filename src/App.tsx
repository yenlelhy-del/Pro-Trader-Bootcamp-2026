...
export default function App() {
  ...
  const GOOGLE_SHEETS_WEBHOOK_URL = 'https://script.google.com/macros/s/AKfycbwFTuZBCLk6FyM8jQtWd3PVL8XFzD5yvUDyd4oKf1LZm45hzhZlWwg1tzmk1FRXl5W5Pw/exec';
  ...
  const handleRegisterSuccess = (name: string, phone: string, brokerCode: string) => {
    ...
    // Gửi lead lên Google Sheets qua webhook Apps Script nếu được cấu hình
    if (GOOGLE_SHEETS_WEBHOOK_URL) {
      // Xác định tên Môi giới dựa trên Broker Code
      let brokerName = 'Khác';
  ...
  return (
    <div className="min-h-screen bg-brand-bg text-[#dbe5de] flex flex-col font-sans select-none overflow-x-hidden selection:bg-brand-mint selection:text-brand-bg">
      ...
      {registeredUser && (
        <div className="bg-[#00e1a1] py-2.5 px-4 text-center text-xs font-display flex items-center justify-center space-x-2 text-[#080706] font-bold animate-fade-in relative z-40 shadow-[0_4px_20px_rgba(0,225,161,0.2)]">
          <User className="w-4 h-4 text-[#080706]" />
          <span>
            Chào mừng thành viên <strong className="text-[#080706] uppercase font-black">{registeredUser.name}</strong> đã tham gia thử thách kỷ luật Pro Trader 2026!
          </span>
          <span className="hidden sm:inline-block px-1.5 py-0.5 bg-[#080706] text-[#00e1a1] text-[9px] font-black rounded ml-2 uppercase tracking-wider">
